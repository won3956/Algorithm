...
/* 	1. 원자의 최초 위치는 2차원 평면상의 [x, y] 이다.
	...
	시뮬레이션 
	1. 0.5초 이동 => 좌표 스케일링
	2. 부딫히는 원자 확인 => x,y 좌표가 같은 원자 => 탐색 시간 줄이기 필요: x,y 좌표를 String으로 변환하여 해시맵
	3. 종료 조건 = 최대 2000초(4000) or 모든 원자가 소멸
...
public class Solution {
	static final int BOUDARY = 2000;
	static int N, result;
	static int[] dy = { 1, -1, 0, 0 };
	static int[] dx = { 0, 0, -1, 1 };
	static Atom[] atoms;
	static Map<Integer, List<Atom>> map;
	...
	public static void move(Atom atom) {
		if(!atom.alive) return;
		atom.x += dx[atom.dir];
		atom.y += dy[atom.dir];
		if(atom.x<-1*BOUDARY || atom.x > BOUDARY || atom.y<-1*BOUDARY || atom.y > BOUDARY) {
			atom.alive = false;
		}
	}
